...
![AUR version](https://img.shields.io/aur/version/mint-backgrounds-una?label=AUR)
[![Build Status](https://drone.cryptic.systems/api/badges/volker.raschek/mint-backgrounds-una-pkg/status.svg)](https://drone.cryptic.systems/volker.raschek/mint-backgrounds-una-pkg)
![AUR votes](https://img.shields.io/aur/votes/mint-backgrounds-una)

This repository contains build files to build the Arch Linux package `mint-backgrounds-una`. Instead of building the
...
```bash
git clone https://aur.archlinux.org/mint-backgrounds-una.git
cd mint-backgrounds-una
makepkg
```
...
```bash
yay --sync --aur mint-backgrounds-una
```
...
sudo pacman --sync mint-backgrounds-una
```
